...
class FwdOnlineLRU(nn.Module):
    ...
    def setup(self):
        self.nu_log = self.param(
            "nu_log", nu_log_init, (1, self.d_hidden), self.r_max, self.r_min
        )
        self.theta_log = self.param(
            "theta_log", theta_log_init, (1, self.d_hidden), self.max_phase
        )
        self.gamma_log = self.param(
            "gamma_log", gamma_log_init, (1, self.d_hidden), self.nu_log, self.theta_log
        )
    ...
    def __call__(self, carry, inputs):
        ...
        new_grad_B = (jnp.expand_dims(Lambda, axis=-1)) * grad_memory[2] + jax.vmap(
            lambda u: jnp.outer(self.gamma_log, u)
...
class OnlineLRUCell(nn.Module):
    ...
    def __call__(self, carry, x_t):
        ...
        def bwd(residuals, y_t):
            ...
            d_params_rec = vjp_to_lambda(
                jnp.expand_dims(jnp.sum(d_output_d_lambda, 0), 0)
            )[0]
            ...
            grad_B = jnp.sum(
                (jnp.expand_dims(d_output_d_h, -1) * new_grad_memory[2]), 0
            )
            params_t1 = flax.core.unfreeze(params_t)
            params_t1["params"]["nu_log"] = correct_nu_log
            params_t1["params"]["theta_log"] = correct_theta_log
            params_t1["params"]["gamma_log"] = (
                jnp.expand_dims(jnp.sum(correct_gamma_log, 0), 0)
            ).real
...
if __name__ == "__main__":
    ...
    h_init, grad_momory_init = OnlineProjLRULayer.initialize_state(
        batch_size, d_hidden, input_dim
    )
