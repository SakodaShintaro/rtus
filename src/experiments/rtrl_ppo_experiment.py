...
def env_step(runner_state, unused):
    ...
    action, value, log_prob, agent_state = agent_step(agent_state, last_obs, last_done,_rng)
    # env step
    rng, rng_step = jax.random.split(rng)
    obs, env_state, reward, done, info = gymnax_state.env_step(rng_step, gymnax_state.env_state, action.squeeze(), gymnax_state.env_params)
    
    # store transition
    transition = Transition(in_hstate,last_obs,last_done, action.squeeze(), value.squeeze(), reward, log_prob.squeeze(),info)
    gymnax_state = GymnaxEnvState.create(env_step=gymnax_state.env_step,env_params=gymnax_state.env_params,env_state=env_state)
...
def experiment_step(runner_state, unused):
    ...
    # Update
    agent_state, rng, loss_info = agent_update(agent_state, traj_batch,last_obs, last_done,rng)
...
def experiment(rng,config:ExpConfig):
    ...
    gymnax_state = GymnaxEnvState.create(env_step=env.step,env_params=env_params,env_state=env_state)
    ...
    rng, _rng = jax.random.split(rng)
    agent_state = init_agent_state(ppo_config, action_dim, env_config.continous_action,obs_shape, _rng)
    
    # start the experiment
    runner_state = (agent_state, gymnax_state, obs,False, rng)
    runner_state, metric = jax.lax.scan(experiment_step, runner_state, None, ppo_config.num_updates)
    return {"runner_state": runner_state, "metrics": metric}
   
def main():
    # Reading config file
    parser = argparse.ArgumentParser()
    parser.add_argument('--config_file',type=str,default='configs/pobrax_config_rtu_rtrl.yaml')
