...
def exp_step(runner_state, unused, env):
    ...
    def interacton_step(interaction_state, unused):
        ...
        action, value, log_prob = agent_step(
            agent_state, jnp.expand_dims(last_obs, (0, 1)), _rng
        )
        # env step
        rng, rng_step = jax.random.split(rng)
        obs, env_state, reward, done, info = env.step(
            rng_step, env_state, action.squeeze(), env_params
        )
        # save transition
    ...
    runner_state, traj_batch = jax.lax.scan(
        interacton_step, runner_state, None, rollout_steps
    )
    ...
    _, last_value = train_state.apply_fn(
        train_state.params, jnp.expand_dims(last_obs, (0, 1))
    )
...
def experiment(rng, config: ExpConfig):
    ...
    agent_state = init_agent_state(
        ...
        env_config.continous_action,
        _rng,
    )

    runner_state = (agent_state, env_state, env_params, obs, rng)
    runner_state, metric = jax.lax.scan(
        partial(exp_step, env=env), runner_state, None, ppo_config.num_updates
    )
...
def main():
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="experiments_configs/test_configs/gymnax_config.yaml",
    )
