...
class RealTimeActorCritic(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        hidden, shared_repr = rec_fn(self.config.d_rec)(
            hidden, shared_repr
        )  # hidden: (batch_size, d_hidden) , shared_repr: (batch_size, d_hidden)

        #### Actor head ####
        actor_mean = nn.Dense(
            self.config.d_actor_head, kernel_init=orthogonal(2), bias_init=constant(0.0)
        )(shared_repr)
        actor_mean = activation(actor_mean)
        actor_mean = nn.Dense(
            self.action_dim, kernel_init=orthogonal(0.01), bias_init=constant(0.0)
        )(actor_mean)  # actor_mean: (batch_size, action_dim)
    ...
    def initialize_state(self, batch_size):
        ...
        carry_init = self.get_rec_fn().initialize_state(
            batch_size, self.config.d_rec, d_input
        )
        return carry_init
    ...
    def check_done(self, carry, dones, batch_size):
        ...
        new_carry = jax.tree_util.tree_map(
            lambda x, y: jnp.where(
                jnp.expand_dims(dones, (np.arange(1, len(x.shape) - 1))), x, y
            ),
            temp_carry,
            carry,
        )
