...
def init_agent_state(ppo_config: PPOConfig, action_dim:int, continous_action:bool,obs_shape:tuple, rng):
    ...
    return AgentState(ppo_config=ppo_config, network_state=network_state, last_hstate=init_hstate)

@jax.jit 
def agent_step(agent_state: AgentState, obs_t:jnp.ndarray, done_t:jnp.ndarray,rng: PRNGKey):
    rnn_in = (jnp.expand_dims(obs_t,0),jnp.expand_dims(done_t,(0,1)))
    last_hidden,pi, value = agent_state.network_state.apply_fn(agent_state.network_state.params, agent_state.last_hstate,rnn_in)
...
@jax.jit  
def agent_update(agent_state: AgentState, traj_batch: list,last_obs, last_done,rng: PRNGKey):
    # Calculate the advantages and value targets
    advantages, targets = _calculate_targets(agent_state, traj_batch, last_obs, last_done)
    # Learning 
    update_state = (agent_state,traj_batch, advantages, targets,last_obs,last_done, rng)
    update_state, loss_info = jax.lax.scan(_update_step, update_state, None, agent_state.ppo_config.epochs)
...
def _update_step(update_state, unused):
    agent_state, traj_batch, advantages, targets,last_obs,last_done, rng = update_state
    batch = (traj_batch, advantages, targets)
    # Prepare minibatches
    minibatches_info,rng = _create_minibaches(agent_state,batch, rng)
    # TODO: if bptt or bptt_rtrl -> don't do the squeeze
    minibatches_info = jax.tree_util.tree_map(
        lambda x: x.squeeze(2), minibatches_info) 
    
    # Loop through minibatches 
    agent_state, (total_loss,grad_norm) = jax.lax.scan(_update_minbatch, agent_state, minibatches_info)
    update_state = (agent_state, traj_batch, advantages, targets,last_obs,last_done, rng)
    
    
    refresh_hstates_and_maybe_targets = not agent_state.ppo_config.stale_gradient 
    refresh_target_only = not agent_state.ppo_config.stale_target and agent_state.ppo_config.stale_gradient
    

    update_state = jax.lax.cond(refresh_target_only,
                                _refresh_targets_only,
                               lambda x:x,
                               update_state)
    
        
        
    update_state = jax.lax.cond(refresh_hstates_and_maybe_targets,
...
def _refresh_targets_only(update_state):
    ...
    hidden_states  = jax.tree_util.tree_map(lambda x: x.squeeze(1), traj_batch.hidden_state)
    _,_, value = agent_state.network_state.apply_fn(agent_state.network_state.params, hidden_states,rnn_in)
...
def _refresh_hstates_and_maybe_targets(update_state):
    ...
    def _get_hiddens(in_hidden, inputs):
        obs,done,_ = inputs
        hiddens,_, value = agent_state.network_state.apply_fn(agent_state.network_state.params, in_hidden,(obs,done))
        return hiddens, (hiddens,value)
    # refresh the hiddens states
    in_hidden = jax.tree_map(lambda x: x[0,:],traj_batch.hidden_state)
    _, (hstates,value) = jax.lax.scan(_get_hiddens, in_hidden, (jnp.expand_dims(traj_batch.obs,1),jnp.expand_dims(traj_batch.done,(1,2)),traj_batch.action)) 
    ...
    (traj_batch, advantages, targets)  = jax.lax.cond(agent_state.ppo_config.stale_target,
                                                      lambda x: x,
                                                      _update_targets,
...
def _update_minbatch(agent_state, batch_info):
    ...
    total_loss, grads = grad_fn(
        train_state.params,train_state.apply_fn, batch_info,
...
def _create_minibaches(agent_state,batch, rng):
    ...
    config = agent_state.ppo_config
    hidden_state = jax.tree_util.tree_map(lambda y:jnp.squeeze(y,axis=1),traj_batch.hidden_state) 
...
def _ppo_rnn_loss_fn(params,agent_fn, minibatch,clip_eps,vf_coef,ent_coef):
    ...
    return total_loss, (value_loss, loss_actor, entropy,k1,k2,k3,ratio.mean(),explained_variance.mean())
...
def _calculate_targets(agent_state, traj_batch, last_obs, last_done):
    ...
    _,_, last_value = agent_state.network_state.apply_fn(agent_state.network_state.params, agent_state.last_hstate,rnn_in)
    last_val = last_value.squeeze()
    
    ## Calculate targets and advantages
    advantages, targets = _calculate_gae(traj_batch, last_val,last_done,agent_state.ppo_config.gamma,agent_state.ppo_config.gae_lambda)
...
def _calculate_gae(traj_batch, last_val,last_done,gamma,gae_lambda):
    ...
    def _get_advantages(carry, transition):
        ...
        gae = (
            delta
            + gamma * gae_lambda * (1 - next_done) * gae
        )
...
@jax.jit
def tree_norm(x):
    return jnp.sqrt(jax.tree_util.tree_reduce(jnp.add, jax.tree_map(lambda x: jnp.sum(x**2), x)))

@jax.jit
def cos_sim_pytrees(old_params,new_params):
    mul = jax.tree_map(lambda x,y: x*y,old_params,new_params)
    dot_prod = jax.tree_util.tree_reduce(jnp.add, jax.tree_map(lambda x: jnp.sum(x), mul))
