...
def agent_step(agent_state: AgentState, obs: jnp.ndarray, rng: PRNGKey):
    ...
    obs.shape = (obs_dim,)
    """
    pi, value = agent_state.network_state.apply_fn(
        agent_state.network_state.params, jnp.expand_dims(obs, (0, 1))
    )
...
def update_step(traj_batch, agent_state, last_val, rng):
    ...
    agent_state, loss_info, rng = jax.lax.cond(
...
def _ppo_loss_fn(
    params, agent_fn, traj_batch, gae, targets, clip_eps, vf_coef, ent_coef
):
...
def _update_step_stale_target(update_data):
    ...
    update_state, loss_info = jax.lax.scan(
        _batch_update, update_state, None, config.epochs
    )
...
def _update_step_fresh_target(update_data):
    ...
    update_state = (
        ...
        last_val,
        rng,
        agent_state.ppo_config,
    )
    config = agent_state.ppo_config
    update_state, loss_info = jax.lax.scan(
        _update_epoch_fresh_targets, update_state, None, config.epochs
    )
    agent_state = AgentState(ppo_config=config, network_state=update_state[0])
    return agent_state, loss_info, update_state[-2]
